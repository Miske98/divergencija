...
def compute_thresholds(emb):
    ...
    l_max = (1 + np.sqrt(beta))**2
    # 2. Gavish-Donoho (Universal Hard Threshold)
    # Omega aproksimacija za nepoznat šum
    omega = 0.56 * beta**3 - 1.28 * beta**2 + 1.10 * beta + 1.28
    gd_thresh = omega * np.median(la.svdvals(emb))**2 / N # Aproksimacija za eig
    # 3. Parallel Analysis (Permutacija)
    rnd_emb = np.apply_along_axis(np.random.permutation, 1, emb)
    pa_thresh = la.eigvalsh(np.corrcoef(rnd_emb)).max()
...
def apply_advanced_filter(embeddings, n_top, n_bottom, method):
    ...
    # Automatsko čišćenje repa prema metodu
    if method == "RMT": evals_filt[evals < l_max] = np.mean(evals[evals < l_max])
    elif method == "Gavish-Donoho": evals_filt[evals < gd_t] = 0
    elif method == "Parallel Analysis": evals_filt[evals < pa_t] = 0

    # Manuelni Skalpel (uvek aktivan povrh svega)
    ...
    return clean_corr, evals, evals_filt, (l_max, gd_t, pa_t)
...
with st.sidebar:
    ...
    input_raw = st.text_area("Unos podataka (red po red):", "Kralj\nKraljica\nMuškarac\nŽena\nJabuka\nKruška\nNarandža\nAvion\nAuto\nBrod", height=250)
...
if "obj" in st.session_state:
    ...
    with col_f:
        ...
        c_top = st.slider("Ukloni najveće (Top-N)", 0, n-1, 1) # Default 1 jer je najsmislenije
        c_bottom = st.slider("Ukloni najmanje (Bottom-N)", 0, n-1, 0)
    
    clean_c, ev_raw, ev_f, threshs = apply_advanced_filter(d["emb"], c_top, c_bottom, auto_method)
    ...
    with col_s:
        ...
        fig_s.update_layout(height=300, margin=dict(t=20, b=20), yaxis_type="log", title="Scree Plot (Log skala)")
    ...
    with res_c2:
        ...
        try:
            ...
            if not np.any(np.isnan(dist_m_s)):
    ...
    st.metric("Srednja distanca", f"{np.mean(dist_m[np.triu_indices(n, k=1)]):.3f}")
